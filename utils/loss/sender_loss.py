...
class Sender_loss(nn.Module):
    def __init__(self, __C):
        ...
        self.USE_CLIP = __C.SENDER['USE_CLIP']
        ...
        if self.USE_CLIP:
            ...
            self.clip_sem_loss_func = nn.CosineSimilarity(dim=1, eps=1e-06)
    ...
    def cal_clip_loss(self, imgs, sketches):
        ...
        for i in range(len(imgs_clip_feats)):
            clip_loss_item = self.clip_loss_func(imgs_clip_feats[TRANFORMER_RESBLOCKS_KEYS[i]], sketches_clip_feats[TRANFORMER_RESBLOCKS_KEYS[i]])
            if clip_loss_item < 0.99: 
                clip_loss += clip_loss_item
    ...
    def cal_bw_thre_loss(self, comm_mask, comm_ratio = 0.05, sketches_raw=None):
        bs, _, h, w = comm_mask.shape
        sum_pixels = h * w
        comm_loss_sum = 0.0
        non_com_mask = 1 - comm_mask
        comm_loss = (sketches_raw * non_com_mask).mean()/bs
        return comm_loss
        # for i in range(bs):
        ...
        #         comm_loss_sum += 0
        #     else:
        #         comm_loss_sum += ((comm_loss- (sum_pixels * comm_ratio)) / sum_pixels) * 10
        #     return comm_loss_sum * self.lambda_comm

    def forward(self, imgs, sketches, sketches_raw, depth=0, comm_mask=0, comm_ratio=0.05, comm_thre=0.05):
        ...
        if self.BW_LIMIT:
            comm_loss = self.cal_bw_thre_loss(comm_mask, comm_thre*2, sketches_raw)
            # comm_loss = 0
