...
def ans_load(ans_list):
    aid_to_ans = {}
    
    for ans in ans_list:
        aid = str(ans['question_id'])
        aid_to_ans[aid] = ans
...
def my_collate(batch):
    image_feats_iter = torch.stack([item[0] for item in batch], dim = 0)
    ques_ix_iter = torch.stack([item[1] for item in batch], dim = 0)
    ans_iter = torch.stack([item[2] for item in batch], dim = 0)
    images_iter = [item[3] for item in batch]
    return [image_feats_iter, ques_ix_iter, ans_iter, images_iter]